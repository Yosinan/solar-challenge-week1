#!/usr/bin/env python3
"""
Script to generate complete EDA notebooks for all countries
This creates notebooks with all required visualizations from Task 2
"""

import json
import os

def create_eda_notebook(country_name, data_file):
    """Create a complete EDA notebook for a country"""
    ...
    notebook["cells"].append({
        ...
        "source": [
            ...
            "\n",
            "# load data\n",
            f"df = pd.read_csv('../data/{data_file}', parse_dates=['Timestamp'])\n",
    ...
    notebook["cells"].append({
        ...
        "metadata": {},
        "source": [
            f"# {country_name.title()} - Exploratory Data Analysis\n",
    ...
    notebook["cells"].append({
        ...
        "outputs": [],
        "source": [
            f"# Export cleaned DataFrame\n",
            f"df_clean.drop(columns=['Month', 'Hour', 'WD_bin'], errors='ignore').to_csv('../data/{country_name.lower()}_clean.csv', index=False)\n",
            f"print('Cleaned data exported to ../data/{country_name.lower()}_clean.csv')\n",
            f"print('Final shape:', df_clean.shape)"
        ]
    })
...
def main():
    """Generate notebooks for all countries"""
    import os
    ...
    for country_name, data_file in countries:
        print(f"Generating notebook for {country_name}...")
        notebook = create_eda_notebook(country_name, data_file)
        
        notebook_path = os.path.join(notebooks_dir, f"{country_name}.ipynb")
